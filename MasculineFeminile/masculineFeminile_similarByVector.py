...
input_path = "masculine_feminile_csv.csv"
df = pd.read_csv(input_path)

feminine = df['feminine'].tolist()
masculine = df['masculine'].tolist()

matrix = np.zeros((len(feminine), len(masculine)), dtype=int)

for i in tqdm(range(len(feminine)), desc="Processing rows"):
    comp_i = feminine[i]
    pos_i = masculine[i]
...
index_labels = [f"{c}-{p}" for c, p in zip(feminine, masculine)]
result_df = pd.DataFrame(matrix, index=index_labels, columns=masculine)
...
plt.title("Feminine-Masculine forms Comparison Matrix with similar_by_vector")
plt.xlabel("Masculine forms")
plt.ylabel("Feminine-Masculine forms")
